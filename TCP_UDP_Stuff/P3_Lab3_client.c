...
int main(int argc, char** argv) {
    ...
    SOCKET srv_clnt_tcp, clnt_target_udp, max_fd;
    struct sockaddr_in server, from, target;
    struct hostent *hp;
    ...
    for (index1 = 0; index1 < target_count; index1++) {
        ...
        targets[index1].sin_addr.s_addr = inet_addr(buf);
    ...
    FD_SET(clnt_target_udp, &main_rfds_set);
    max_fd = (srv_clnt_tcp < clnt_target_udp) ? clnt_target_udp : srv_clnt_tcp;
    ...
    while (true) {
        ...
        if (FD_ISSET(0, &curr_rfds_set)) {
            ...
            if (strcmp(msg, "QUIT") == 0) {
                ...
                for (index1 = 0; index1 < target_count; index1++) {
                    ...
                    bs = sendto(clnt_target_udp, msg, 200, 0, (struct sockaddr*)&targets[index1], l);
                }
                bs = send(srv_clnt_tcp, msg, 200, 0);
            }
        }
        ...
        if (FD_ISSET(clnt_target_udp, &curr_rfds_set)) {

            // Accepting messages from other clients via udp.
            memset(&from, 0, l);
            memset(msg, 0, 200);
            br = recvfrom(clnt_target_udp, msg, 200, 0, (struct sockaddr*)&from, &l);
